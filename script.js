...
function calcAll() {
  ...
  for (var i = 0; i < a.names.length; i++) {
    ...
    persons[n.id] = { meals: pm, deposited: dep, extra: ext, cost: 0, txn: 0 };
    totalMeals += pm;
  }
  ...
  for (var ii = 0; ii < a.names.length; ii++) {
    var nn = a.names[ii];
    var p = persons[nn.id];
...
function showExpenseModal(idx) {
  ...
  modal.innerHTML = '<div class="modal-c">' +
    ...
    '<div class="field"><label>Category *</label><input id="expCat" placeholder="e.g. Rice, Fish" value="' + (exp ? exp.category : '') + '"></div>' +
    '<div class="field"><label>Description</label><input id="expDesc" value="' + (exp && exp.description ? exp.description : '') + '"></div>' +
  ...
  setTimeout(function() {
    ...
    if (saveBtn) {
      saveBtn.addEventListener('click', function() {
        ...
        var descIn = document.getElementById('expDesc');
        ...
        if (!date || !cat) {
          alert('Date and category required');
          return;
        }
        if (isNaN(amt) || amt < 1) {
        ...
        var obj = { date: date, category: cat, amount: amt };
        if (desc) obj.description = desc;
